...
namespace Quoridor
{
    public sealed class AnyTileLegalRule : ISkillLegalRule
    {
        public SkillLegalRuleId RuleId => SkillLegalRuleId.AnyTile;
        ...
        public bool IsLegal(SkillLegalContext context)
        {
            ...
            return BoardGeometry.IsInside(board, target)
                && BoardGeometry.IsTilePosition(target);
        }
        ...
        )
        {
            ...
            {
                if (BoardGeometry.IsTilePosition(position))
                {
