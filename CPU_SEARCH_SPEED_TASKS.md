...
- [x] `AlphaBetaCpuAgentStrategy` の探索開始時に `SearchProfiler.Begin()`、終了時に `End()` を呼ぶ。
- [x] root、内部ノード、評価呼び出しで `RecordNode()` を呼び、深さごとのノード数をログに残す。
- [x] `Pathfinder` 経由の BFS 回数が探索 1 手単位で見えるよう、既存 DI 経路で profiler を渡す。
- [ ] 初期局面・代表局面で深さ 1〜3 のベースラインを保存する。
- [x] 計測ログには `depth`, `nodes`, `bfs`, `gcAlloc`, `elapsedMs`, `bestScore`, `timeout` を含める。
...
- [x] 探索用 `SearchPathfinder` を追加する。
- [x] `int[,] distance`、`bool[,] visited`、`Queue<Position>` の毎回生成をやめ、一次元配列と queue バッファを再利用する。
- [x] 世代カウンタ方式で盤面全体の初期化を避ける。
- [x] `Pathfinder` と `SearchPathfinder` の到達可否・最短距離が一致するテストを追加する。
- [x] 壁合法性判定では既存壁との衝突や盤外チェックを BFS 前に終わらせる。
